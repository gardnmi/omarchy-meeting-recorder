...
Automatic stopping is a separate, default-off Preferences switch. It associates each new recording (manual or automatic) with the single detected meeting window and stops through the normal save/transcription flow after that window has been closed or shown a recognized ended state for 30 seconds. A reappearing window cancels the countdown. Tab switches, ambiguous meetings at recording start, and failed window queries do not trigger stopping. Turning the switch off cancels monitoring for the current recording; enabling it applies to subsequent recordings. This association lives in the recorder, so restarting the background auto-start watcher does not lose it. Recovered recordings are not automatically associated.

**Google Meet limitation:** clicking Leave can retain the same title, and closing a tab inside a still-open browser is indistinguishable from switching tabs. In these cases, close the meeting window or stop manually. The grace period protects brief window disappearances; a reconnection that creates a different window is not identifiable as the same session. Native Zoom returning to its recognized home title can signal an end, but title formats vary. Live provider testing remains necessary.

 A handled meeting is suppressed for two hours after its last automatic-start attempt. During that time, manual Stop, switching browser tabs, and restarting the watcher do not restart the same call. The deadline is persisted and does not slide while the window stays visible. After expiry, a still-visible meeting window can trigger another recording if the recorder is idle—even if it is a leftover or preview tab. A busy recorder is left uninterrupted; a skipped attempt begins another two-hour suppression period. A different Meet code or name, or a closed/reopened window, can trigger another recording. A title change is a new identity when no code is visible. When available, a Google Meet code or numeric Zoom meeting ID distinguishes rooms with identical names. Dedicated web-app launch metadata can supply this ID; it may become stale if the app navigates to another room. A visible Meet code takes precedence. Recurring meetings that reuse a room ID still use the two-hour suppression timeout. Without an exposed ID, same-name rooms in the same window remain indistinguishable. A meeting skipped because the recorder was busy is not retried automatically. A definite launcher failure, before any recording command is sent, is retried after 30 seconds. The watcher uses the installed executable path even if its running binary was replaced by an update.
