...
Stop recording manually when finished. Leaving a call does not stop recording. Handled meetings are remembered for the desktop session, so manual Stop, switching browser tabs, and restarting the watcher do not restart the same call. A different Meet code or name, or a closed/reopened window, can trigger another recording. A title change is a new identity when no code is visible. Separate calls with the same visible name in the same browser window cannot be distinguished; close and reopen the meeting window between those calls. A meeting skipped because the recorder was busy is not retried automatically.
